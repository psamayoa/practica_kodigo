...
<body>
    <?php include "./modulos/header.php"; 
        ...
        $coaches = new Coach();
        $arreglo_coach = $coaches->obtenerCoach();
        $arreglo_estado = $coaches->estadoByAsincronoActivoDesercion();
        $arreglo_materias = $coaches->getMaterias();
    ?>
    
    <main id="main">
        <section class="container">
            <h1>Gestion de Profesores Activos</h1>
                    ...
                    <?php foreach($arreglo_coach as $item) { ?>
                        <tr>
                            ...
                            <td>
                                <form action="./actualizar_coach.php" method="post">
                                    <!-- input que captura el id de cada coach -->
                        ...
                        <div class="modal fade" id="ModalReubicacion<?php echo $item['id']; ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                <div class="modal-header">
                                    <h1 class="modal-title fs-5" id="exampleModalLabel">Reubicar Coach</h1>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <form action="" method="POST">
                                    <div class="modal-body">
                                        ...
                                        <input type="hidden" name="id_coach" value="<?php echo $item['id']; ?>">
                                        <h5><?php echo $item['nombre']; ?></h5>
                                        <p><strong>Matería Actual: </strong> <?php echo $item['materia']; ?></p>
                                    ...
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
                                        <input type="submit" class="btn btn-danger" value="Reubicar Coach">
                                    </div>
                                </form>

                                <?php $coaches->actualizarReubicacion(); ?>
                                </div>
                            </div>
                        </div>
                    <?php } ?>
